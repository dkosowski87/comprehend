...
def normalize_wiki_latex(latex: str) -> str:
    ...
    This rewrites ``\\bm`` to ``\\mathbf`` (Bishop PRML bold-Roman convention) and other
    unsupported forms to compatible ones before emitting ``$$...$$`` blocks.
    ...
    normalized = re.sub(r"\^\\top\b", r"^{\\mathrm{T}}", normalized)
